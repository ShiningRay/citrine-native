...
| **N5** | dogfooding + 发布准备：移植一个真实应用（候选：beryl 的某个面板或 market-terminal 的简化版） | gem 0.1.0 发布（Trusted Publishing 沿用主仓 OIDC 模式） | 🟡 **准备完成，待发布**（2026-09-15）：dogfooding 已由两个真实应用满足（citrine-sheets / citrine-market-terminal 的原生版，都在 macOS 与 Windows 上跑通）；发布准备就绪——版本定 **0.1.0**（`version.rb`）、`CHANGELOG.md`（Keep a Changelog）、`.github/workflows/release.yml`（Trusted Publishing OIDC + 标签与 VERSION 一致性校验 + 发布门禁）、gemspec 元数据（source/changelog URI）、名称占用已查（`citrine-native` 未被占用）；本机构建并**解包即用**验证（`gem build` → `gem unpack` → `require` 成功）；**发布手册 [RELEASING.md](RELEASING.md)**（一次性前置字段表 → 发布步骤 → 发布后验证 → 出错/撤回处置 → 首次发布的五项人工确认）；**发布元数据的机器对拍 `test/release_metadata_test.rb`（9 项）** 把"版本号 ↔ CHANGELOG ↔ gemspec ↔（提交里的）锁文件 ↔ 工作流的 Ruby 版本 ↔ 工作流触发条件/OIDC 权限/步骤顺序"互锁；**消费端冒烟 `rake consumer_smoke`（10 项）** 本地构建两个 gem、装进干净 GEM_HOME、在仓库外 require 并渲染（断言加载的是打包产物而非工作树）；验收证据汇总见 [docs/plan/acceptance-0.1.0.md](docs/plan/acceptance-0.1.0.md)（N0–N5 逐条口径 ↔ 证据 + 未验证清单）。**剩下的是仓库管理员的动作**：rubygems.org 注册 Trusted Publisher（owner/repo/workflow 三个字段见 RELEASING §一）→ 打并推 `v0.1.0` 标签 |
...
| 2026-09-15 | **提交推送（D-1①）+ v0.1.0 首发与一次失败复盘**：四个提交落库（框架 / 文档 / 测试 / 发布，34 个条目清零）；CI 首跑 **macos-latest + windows-latest 矩阵全绿**（E1 的"待首次运行确认"关闭）；打 `v0.1.0` 标签触发 Release 工作流——**首跑红在门禁**：release.yml 原来整个 job 跑在 ubuntu-latest，libui 的 GTK 后端在无显示服务器上是 C 层 g_error 直接 abort（"Cannot open display" → Gtk-ERROR，子进程 exitstatus 变 nil），冒烟脚本里 `rescue StandardError` 拦不住，`LIBUI_UNAVAILABLE` 兜底走不到。**修复两处**：① release.yml 的 job 从 ubuntu-latest 挪到 **macos-latest**（真原生后端上跑完整门禁，顺带覆盖 ObjcBridge/AppKit 对照断言；gem 是纯 Ruby 包，构建平台无关；Windows 侧由 CI 矩阵在 main 同一提交上守）；② `libui_scenario.rb` 在 init **之前**按"Linux ∧ DISPLAY/WAYLAND_DISPLAY 均空"预判，输出 LIBUI_UNAVAILABLE 让测试侧跳过（CI 注释里"Linux 真控件路径整体跳过"从此为真） | 按 RELEASING §四处置：门禁失败没有任何产物发布（`gh release list` 确认为空）→ 删标签、修复、重打；本机预发布检查 `consumer_smoke` 10/10 通过后才推标签 |
